...
router.get("/", (req, res) => {
  ...
  `).all();
  res.json(withProductOptions(db, rows, { activeOnly: true }));
});
...
router.get("/all", (req, res) => {
  ...
  `).all();
  res.json(withProductOptions(db, rows));
});
...
router.post("/", (req, res) => {
  ...
  const create = db.transaction(() => {
    ...
    if (nextOptions.groups) replaceProductOptions(db, productId, nextOptions.groups);
    return productId;
  });
...
router.patch("/:id", (req, res) => {
  ...
  if (!nextOptions.ok) return res.status(400).json({ error: nextOptions.error });
  if (nextOptions.groups && !optionIdsBelongToProduct(db, id, nextOptions.groups)) {
  ...
  const update = db.transaction(() => {
    ...
    if (nextOptions.groups) replaceProductOptions(db, id, nextOptions.groups);
  });
  update();
